...
function jumlahTabungan(listHarga, history) {
  let result = {};
  let splitHistoryByDay = history.split(".");
  let arrayOfHistory = splitHistoryByDay.map((el) => {
    return el.split("-");
  });

  for (let i = 0; i < arrayOfHistory.length; i++) {
    let day = arrayOfHistory[i][0];
    if (!result[day]) {
      result[day] = arrayOfHistory[i][1].split(",");
    }
    let price = 0;
    for (let j = 0; j < result[day].length; j++) {
      for (let k = 0; k < listHarga.length; k++) {
        if (result[day][j] == listHarga[k].nama) {
          price += listHarga[k].harga;
        }
      }
    }
    result[day] = 10000 - price;
  }
